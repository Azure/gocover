...
`gocover` highly relies on cover profiles generated by `go test`. `go test` generates cover profiles for a specific package at circumstance that there is at least one `_test.go` in the package.
...
So there would be two situations:
1. A package has `_test.go` file. `go test` can generate cover profile demandly, then `gocover` will generate diff coverage for the package.
2. A package has no `_test.go` file, `go test` won't generate any cover profile for the package, `gocover` won't generate any diff coverage for the package.
